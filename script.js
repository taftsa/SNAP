...
$(document).on('click', '.notDone', function() {
	...
	//Praxis 1 required before candidacy
	if ($(this).children('.topBadge').hasClass('praxis1') && !$('#praxis1').hasClass('done')) {
		alert ('Praxis I (Core) required before applying for candidacy.');
		allowRegistration = false;
	};
	
	//45 credits required before candidacy
	if ($(this).attr('id') == 'tep' && $('#totalCredits').html() < 45) {
		alert ('45 credits required before applying for candidacy.');
		allowRegistration = false;
	};
	...
	//TEP candidacy required before advanced ed classes and student teaching
	if ($(this).children('.badge').hasClass('tep') && !$('#tep').hasClass('done')) {
		alert ('TEP Candidacy required before registering for this class.');
		allowRegistration = false;
	};
	
	//PLT required before student teaching
	if ($(this).children('.badge').hasClass('plt') && !$('#plt').hasClass('done')) {
		alert ('PLT required before registering for this class.');
		allowRegistration = false;
	};
	
	//Praxis 2 required before student teaching
	if ($(this).children('.badge').hasClass('praxis2') && !$('#praxis2').hasClass('done')) {
		alert ('Praxis II (Content Knowledge) required before registering for this class.');
		allowRegistration = false;
	};
...
$(document).on('click', '.done', function() {
	if ($(this).hasClass('requirement') || $(this).hasClass('techniques')) {
		$(this).removeClass('done');
		$(this).addClass('notDone');
	} else if ($(this).hasClass('slo') || $(this).hasClass('ensembles')) {
		
	} else if ($(this).hasClass('techniques')) {
		$(this).removeClass('done');
		$(this).addClass('notDone');
	} else {
		$(this).removeClass('done');
...
//Highlight Techniques on Hover
$(document).on('mouseenter', '.techniques', function() {
	$('.tech').css('background-color', '#fcffa4');
});

$(document).on('mouseleave', '.techniques', function() {
	$('.tech').css('background-color', '');
});
...
$(document).on('click', '#honors', function(){
	if (!$(this).hasClass('added')) {
		if (confirm('Add Honors?')) {
			...
			$('#5').append('<div class="honorsClass class notDone"><span class="courseCode">HN 423</span><span class="courseTitle">Honors Capstone</span><div class="credits">3</div><div class="honors badge">\uD83C\uDF96</div><div class="courseTime">12:30p TR</div></div>');
